...
(** Function to get the wavenumbers (1D wavevector) for a given 1D
    domain and number of gridpoints. Returns: float list of length n *)
...
(** Computes the 1D second derivative of an array, based on a
    finite-difference method. [second_derivative w b d n] returns a
    Complex.t array of length n, which is the second derivative of w. *)
...
module FreeParticleEvolutionEulers1D = struct
  ...
  (** Same as step, but mutates the array w instead of returning a new
      array. *)
...
(** [get_x n d] returns a list of n floats, evenly spaced between
    [fst d] and [snd d] *)
...
(** [get_k2_2d n m d2] returns a matrix corresponding to the
    x-wavevector squared plus the y-wavevector squared at each
    gridpoint. *)
let get_k2_2d (n : int) (m : int) (d2 : domain2d) =
  build_k2 (get_k2 n (fst d2)) (get_k2 m (snd d2))

(** [probabilities1d vec] returns a lst corresponding to the (complex)
    magnitude squared of each entry of vec. Helper function of
    probabilities *)
...
(** Equivalent of second_derivative for 2d functions (lists). is_x tells
    the function whether to take the derivative w.r.t x (if true), or y
    (if false) *)
